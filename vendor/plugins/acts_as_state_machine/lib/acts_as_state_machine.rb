  ...
  module Acts                        #:nodoc:
    module StateMachine              #:nodoc:
      ...
      module InstanceMethods
        ...
        def valid_events
          (self.class.read_inheritable_attribute(:transition_table).collect do |event, transitions|
            event if transitions.detect {|transition| transition.from == current_state}
          end).compact
        end
        
        def next_states
          (self.class.read_inheritable_attribute(:transition_table).collect do |event, transitions|
            transitions.collect {|transition| transition.to if transition.from == current_state}
          end).flatten.compact
        end
