...
#      )
      # try to handle "ActiveRecord::StatementInvalid (Mysql::Error: Lost connection to MySQL server" ....
      begin
        try_to_set_viewer
  ...
  def try_to_set_viewer
    @viewer = Viewer.find @id#, :include => [ { :persons => :staff }, :projects_coordinator ] # include is giving issues
  end
